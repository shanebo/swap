...
const defaults = () => ({
  maxAge: 5,
  link: 'a[href]:not([target="_blank"]):not([data-swap-ignore]):not([data-swap-confirm]):not([formmethod])',
  button: `
  ...
  confirmTemplate: ({ title, cancel, ok }) => `
    <div class="Confirm">
      <h2>${title}</h2>
      <button data-swap-model-confirm-cancel>${cancel}</button>
      <button data-swap-model-confirm-ok>${ok}</button>
    </div>
  `,
  dialogSelectors: ['.Dialog -> body'],
  notice: '.Notice',
  css: `
