...
import { getPanes, hasPane, loadPane, prevPane, samePane, addPane, closePanes, clickOffPane } from './pane.js';
...
swap.with = (options, selectors = [], callback = openPage) => {
  ...
  ajax(req, (xhr, res, html) => {
    ...
    if (method === 'post' && [loadPane, samePane, prevPane, addPane].includes(callback)) {
      swap.panePostOccurred = true;
    }
...
swap.event = function(name, delegate, fn) {
  const target = (name === 'resize' || name === 'scroll') && arguments.length === 2 ? window : document;
  const e = {
    name,
    target,
  ...
  e.target.addEventListener(e.name, e.fn);
  return swap;
}
...
swap.click = function(e) {
  ...
  if (e.metaKey || !shouldSwap(buildUrl(link))) return;
...
swap.closePane = ({ html, selectors, finalMethod, finalUrl } = {}, formSaved = false) => {
  ...
  if (panes.length >= 2) {
    ...
    if ((!formSaved || fromPaneEdited) && !swap.panePostOccurred) {
      // just closing top pane to reveal previous pane
      prevPane({ finalUrl: fromPaneUrl });
    } else if (fromPaneUrl === getPath(finalUrl)) {
      // reloading previous pane with cached html
      prevPane({ finalUrl: fromPaneUrl, html });
    } else {
      // fully reloading previous panes url
      swap.with(fromPaneUrl, swap.opts.paneSelectors, prevPane);
    }
  } else {
    if (swap.panePostOccurred) {
      // reloading page
...
swap.setPattern = (name, selectors) => {
  swap.patterns[name] = selectors;
}
