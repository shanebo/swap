...
import { resetHash, popState, getPreviousUrl, getCurrentUrl } from './history.js';
...
import { getPaneState, getPanes, hasPane, prevPane, samePane, addPane, closePanes, clickOffPane } from './pane.js';
...
swap.closePane = ({ html, selectors, finalMethod, finalUrl } = {}, formSaved = false) => {
  ...
  if (panes.length >= 2) {
    const { url, edited } = getPaneState(-2);

    if (!formSaved || edited) {
      prevPane({ finalUrl: url });
    } else if (url === getPath(finalUrl)) {
      prevPane({ finalUrl: url, html });
    } else {
      swap.with(url, swap.opts.paneSelectors, prevPane);
    }
  } else {
    if (formSaved) {
