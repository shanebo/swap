...
import { prevPane, continueForm, repeatForm, samePane, addPane, closePanes } from './pane.js';
...
swap.submit = function(e, selectors) {
  ...
  if (swapInline) {
    ...
    swap.redirectTo = currentUrl();
    swap.with(target, sels, repeatForm);
  } else if (target.hasAttribute('data-swap-continue')) {
    swap.redirectTo = previousUrl();
    swap.with(target, sels, continueForm);
  } else {
    const callback = document.documentElement.classList.contains(swap.opts.paneIsOpen)
        ? samePane
        : openPage;
...
//   const isPrevPane = swap.paneHistory.length >= 2;
//   const previousPane = isPrevPane ? swap.paneHistory[swap.paneHistory.length - 2] : {}

//   const shouldReload = swap.paneSaved && !previousPane.edited;
...
//   } else {
//     if (shouldReload) {
//       if (previousPane.url = getPath(finalUrl)) {
//         prevPane(previousPane.url, html, previousPane.selectors);
//       } else {
//         swap.with(previousPane.url, previousPane.selectors, ({ html, finalUrl, selectors }) => prevPane(finalUrl, html, selectors));
//       }
//     } else {
...
swap.closePane = ({ html, selectors, finalMethod, finalUrl } = {}) => {
  if (swap.paneHistory.length >= 2) {
    ...
    //     if (url === getPath(finalUrl)) {
    //       console.log('swap.to');
    //       swap.to(html, selectors);
    //     } else {
    //       console.log('swap.with(url, selectors)');
    ...
    // if (url === getPath(finalUrl)) {
    // if (swap.paneSaved) {
    //   console.log('going to reloadPage');
    //   reloadPage();
    // }
    // closePanes();
  }
