...
const loadSwapStyles = () => {
  const style = document.createElement('style');
  style.append(swap.opts.css);
...
// Parts regex for relative paths to be checked for reloading
...
const buildAssetMap = (el, selector) => [...el.querySelectorAll(selector)]
  ...
  .map((url) => {
    ...
    const matches = pathname.match(assetPartsRegex);

    if (matches) {
      const [ _, name, hash, extension ] = matches;
      return {
