...
const renderBody = (dom, selectors) => {
  ...
  const flash = dom.querySelector(swap.qs.flash);
  const oldFlash = qs(swap.qs.flash);

  const fullSwap = (!selectors.length || (matches.length !== selectors.length)) || (flash && !oldFlash);
  ...
  if (fullSwap) {
    ...
    if (flash) {
      matches.push(render.replace.bind(null, flash, oldFlash));
    }
