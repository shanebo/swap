...
import { getPaneUrl, htmlToElement, getPreviousSibling, getPath } from './utils.js';
...
const addPane = ({ html, finalUrl, selectors }, bypass = false) => {
  ...
  setTimeout(() => {
    ...
    pane.querySelector('[autofocus]')?.focus();
...
const samePane = ({ html, finalUrl }) => {
  ...
  pane.querySelector('[autofocus]')?.focus();
  const paneUrl = location.hash.replace('#pane=', '');
  if (getPath(finalUrl) !== paneUrl) {
    updatePaneHistory(finalUrl);
  }
}
