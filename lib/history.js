...
/*
click/submit/addPane = pushState(timeout)
  replace current session item state with current page state
  swap elements
  push browser state
  push new item to session with current page state
  push pane state (figure this out dynamically)
  if (timeout) after pane is closed (700ms) replace current session item state with current page state
