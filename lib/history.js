...
const updateSessionState = (url) => {
  while(!trySetSessionState() && attempts > 0) {
    deleteOldestHtml();
  }
}


const deleteOldestHtml = () => {
  const stateIds = session.get('stateIds');
  const stateId = stateIds.find((stateId) => {
    const state = session.get(stateId);
  ...
  if (stateId != null) {
    const state = session.get(stateId);
    delete state.html;
    session.set(stateId, state);
  }
}


const trySetSessionState = (url) => {
  try {
